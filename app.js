(() => {
  ...
  const SUBMISSIONS_KEY = "fpl-sidekick-submissions-v1";
  ...
  function saveSubmission(record) {
    try {
      ...
      const remaining = submissions.filter((entry) => entry.id !== record.id);
      remaining.unshift(record);
      localStorage.setItem(SUBMISSIONS_KEY, JSON.stringify(remaining.slice(0, 50)));
      return true;
    } catch {
      return false;
    }
  ...
  function renderAnalysisReport(record) {
    ...
    report.append(makeNode("p", "report-disclaimer", "This is an automated first-pass analysis of the saved official FPL snapshot, not a guaranteed points prediction. A reviewer can add their own advice in the local demo review desk."));
  ...
  async function submitSquad(event) {
    ...
    setSubmissionStatus("Sending your squad to the FPL Sidekick review inbox…");
    ...
    const saved = saveSubmission(record);
    submitButton.disabled = false;
    submitButton.innerHTML = originalButton;
    elements.submissionForm.removeAttribute("aria-busy");
    setSubmissionStatus(
      sent
        ? "Your review request was sent to the inbox. The team can reply to the email address you entered after a human review."
    ...
    if (sent) showToast(saved ? "Your review request was sent." : "Your request was emailed; this browser could not save the local review copy.");
